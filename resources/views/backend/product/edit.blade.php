        ...
        <form method="post" enctype="multipart/form-data" action="{{ route('product.update',$product->id) }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" id="category_id" class="form-control">
                            ...
                            <option value='{{ $cat_data->id }}'
                                {{ (($product->category_id==$cat_data->id)? 'selected' : '') }}>{{ $cat_data->title }}
                            </option>
                            @endforeach
                        </select>
                        @error('category_id')
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="summary"
                    placeholder="Write short description.....">{{ $product->summary }}</textarea>
                @error('summary')
                <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" rows="10" name="description"
                    placeholder="Write detail description.....">{{ $product->description }}</textarea>
                @error('description')
                <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
                    ...
                    <div class="col-md-6">
                        <div class="input-group">
                            ...
                            <label class="custom-file">
                                ...
                                <span class="custom-file-label form-control-input"
                                    for="photo">{{ $product->photo }}</span>
                                @else
